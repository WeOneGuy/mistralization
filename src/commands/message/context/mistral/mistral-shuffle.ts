...
export default {
  ...
  async execute(interaction: MessageContextMenuCommandInteraction<"cached">) {
    ...
    const response = await complete(message.content, "You must shuffle the words in user's message in random order and return the result. Return only the result.");
